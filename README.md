...
`AI Fiction to Script` 是一个基于 Qwen 的模块化改编工具，用于把小说文本转换为结构化、可编辑的 YAML 剧本草稿。
...
- `v0.4.0`
- 已包含 CLI 工作流、本地版本库、Schema 导出、场景重生成，以及可视化 Web 工作台
...
如果你之前已经安装过旧版本的 editable 包，拉取最新代码后建议重新执行一次安装命令，确保 `novel2script web` 等入口与当前源码保持一致。

## Qwen 配置

使用在线 Qwen 提供方前，请先设置 DashScope / Qwen API Key：
...
如果你的账号不在默认地域，需要把 `QWEN_BASE_URL` 配成对应地域的 DashScope OpenAI 兼容地址。
